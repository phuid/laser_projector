...
class PathLine(object):
    ...
    def render(self, params):
        ...
        for i in range(1, steps+1):
            x = self.start[0] + i * dx
            y = self.start[1] + i * dy
            out.append(LaserSample(
                (int(x*params.width), int(y*params.height)), self.on))
            if self.on:
...
class PathBezier3(object):
    ...
    def showinfo(self, tr=''):
        print(tr+'Bezier( %s %s %s )' %
              (pc(self.start), pc(self.cp), pc(self.end)))
...
class LaserPath(object):
    ...
    def render(self, params):
        ...
        out = []
        if is_closed:
            out += [LaserSample((int(sx*params.width),
                                int(sy*params.height)))] * params.closed_start_dwell
        else:
            out += [LaserSample((int(sx*params.width),
                                int(sy*params.height)))] * params.start_dwell
        params.points_dwell_start += params.start_dwell
        for i, s in enumerate(self.segments):
            params.subpaths += 1
            out += s.render(params)
            ex, ey = s.end
            end_render = [LaserSample(
                (int(ex*params.width), int(ey*params.height)))]
...
class LaserFrame(object):
    ...
    def render(self, params):
        ...
        for n, i in enumerate(self.objects):
            ...
            out += i.render(params)
            cpos = out[-1].coord[0] / \
                params.width, out[-1].coord[1] / params.height
            npos = self.objects[(n+1) % len(self.objects)].startpos()
            trip = [LaserSample(
                (int(cpos[0]*params.width), int(cpos[1]*params.height)))] * params.switch_on_dwell
            trip += PathLine(cpos, npos, False).render(params)
            trip += [LaserSample((int(npos[0]*params.width),
                                 int(npos[1]*params.height)))] * params.switch_off_dwell
...
class SVGPath(object):
    ...
    def arc_eval(self, cx, cy, rx, ry, phi, w):
        ...
        x = rx * math.cos(w)
        y = ry * math.sin(w)
        x, y = x*math.cos(phi) - y * \
            math.sin(phi), math.sin(phi)*x + math.cos(phi)*y
        x += cx
        y += cy
        return (x, y)
...
class SVGReader(xml.sax.handler.ContentHandler):
    ...
    def startElement(self, name, attrs):
        if name == "svg":
            ...
            if 'transform' in attrs.keys():
                self.transform(attrs['transform'])
            if self.defsdepth == 0 and self.isvisible(attrs):
                cx, cy, rx, ry = [float(attrs[x])
                                  for x in ('cx', 'cy', 'rx', 'ry')]
    ...
    def addCircle(self, cx, cy, r):
        cp = 0.55228475 * r
        path = LaserPath()
        path.add(PathBezier4((cx, cy-r), (cx+cp, cy-r),
                 (cx+r, cy-cp), (cx+r, cy)))
        path.add(PathBezier4((cx+r, cy), (cx+r, cy+cp),
                 (cx+cp, cy+r), (cx, cy+r)))
        path.add(PathBezier4((cx, cy+r), (cx-cp, cy+r),
                 (cx-r, cy+cp), (cx-r, cy)))
        path.add(PathBezier4((cx-r, cy), (cx-r, cy-cp),
    ...
    def addEllipse(self, cx, cy, rx, ry):
        ...
        cpy = 0.55228475 * ry
        path = LaserPath()
        path.add(PathBezier4((cx, cy-ry), (cx+cpx, cy-ry),
                 (cx+rx, cy-cpy), (cx+rx, cy)))
        path.add(PathBezier4((cx+rx, cy), (cx+rx, cy+cpy),
                 (cx+cpx, cy+ry), (cx, cy+ry)))
        path.add(PathBezier4((cx, cy+ry), (cx-cpx, cy+ry),
                 (cx-rx, cy+cpy), (cx-rx, cy)))
        path.add(PathBezier4((cx-rx, cy), (cx-rx, cy-cpy),
                 (cx-cpx, cy-ry), (cx, cy-ry)))
...
def write_ild(params, rframe, path, center=True):
    ...
    for i, sample in enumerate(rframe):
        if sample.on:
            rframe = rframe[:i] + [sample] * \
                params.extra_first_dwell + rframe[i+1:]
            break
    ...
    for i in range(frames):
        hdr = struct.pack(">4s3xB8s8sHHHBx", b"ILDA", 1,
                          b"svg2ilda", b"", samples, i, frames, 0)
        fout.write(hdr)
        fout.write(dout)
    hdr = struct.pack(">4s3xB8s8sHHHBx", b"ILDA", 0,
                      b"svg2ilda", b"", 0, 0, frames, 0)
    fout.write(hdr)
    fout.close()
