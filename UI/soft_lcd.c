...
lcd_t *lcd_create(int pi, unsigned bus, int addr, int lines)
{
	...
	int i2c_handle = lgI2cOpen(bus, addr, 0);
	if (!_pcf8574_check(i2c_handle, addr))
		return NULL;
...
void lcd_backlight_dim(lcd_t *lcd, uint8_t intensity)
{
	if (lcd->_dimming < 0)
	{
		lgGpioClaimOutput(lcd->_pi, LCD_PWM_PIN, LG_SET_PULL_NONE, 0);
	}
	...
	lgTxPwm(lcd->_pi, LCD_PWM_PIN, 100, intensity, 0, 0);
...
void _pcf8574_put(lcd_t *lcd, int lines)
{
	...
	if (
			(lgI2cWriteByte(lcd->_i2c_handle, lcd->_addr << 1 | I2C_WRITE) == I2C_ACK) &&
			(lgI2cWriteByte(lcd->_i2c_handle, lines | LCD_ENABLED) == I2C_ACK) &&
			(lgI2cWriteByte(lcd->_i2c_handle, lines) == I2C_ACK))
	{
		return;
	}
...
int _pcf8574_get(lcd_t *lcd, int rs)
{
	...
	lgI2cWriteByte(lcd->_i2c_handle, lcd->_addr << 1 | I2C_WRITE);
	lgI2cWriteByte(lcd->_i2c_handle, lines);
	lgI2cWriteByte(lcd->_i2c_handle, lines | LCD_ENABLED);

	/* Actually read lines */
	lgI2cWriteByte(lcd->_i2c_handle, lcd->_addr << 1 | I2C_READ);
	byte = lgI2cReadByte(lcd->_i2c_handle);
	...
	lgI2cWriteByte(lcd->_i2c_handle, lcd->_addr << 1 | I2C_WRITE);
	lgI2cWriteByte(lcd->_i2c_handle, lines & ~LCD_ENABLED);
	lgI2cWriteByte(lcd->_i2c_handle, lines & ~LCD_READ);
...
int _pcf8574_check(int i2c_handle, int addr)
{

	int r = lgI2cWriteByte(i2c_handle,
												 addr << 1 | I2C_WRITE);
