...
lcd_t *lcd_create(int pi, unsigned bus, int addr, int lines)
{
	...
	int i2c_handle = i2c_open(pi, bus, addr, 0);
	if (!_pcf8574_check(pi, i2c_handle, addr))
		return NULL;
...
void lcd_print(lcd_t *lcd, char *instr)
{
	...
	if (lcd->replace_UTF8_chars)
		s = _replace_UTF8_chars(instr);
...
void lcd_backlight_dim(lcd_t *lcd, float intensity)
{
	if (lcd->_dimming < 0)
	{
		set_mode(lcd->_pi, LCD_PWM_PIN, PI_OUTPUT);
		set_PWM_range(lcd->_pi, LCD_PWM_PIN, LCD_PWM_RANGE);
	}
	...
	set_PWM_dutycycle(lcd->_pi, LCD_PWM_PIN, (int)(intensity * (float)LCD_PWM_RANGE));
...
void _pcf8574_put(lcd_t *lcd, int lines)
{
	...
	if (
			(i2c_write_byte(lcd->_pi, lcd->_i2c_handle, lcd->_addr << 1 | I2C_WRITE) == I2C_ACK) &&
			(i2c_write_byte(lcd->_pi, lcd->_i2c_handle, lines | LCD_ENABLED) == I2C_ACK) &&
			(i2c_write_byte(lcd->_pi, lcd->_i2c_handle, lines) == I2C_ACK))
	{
		return;
	}
...
{
	int byte;
	int lines = LCD_D4 | LCD_D5 | LCD_D6 | LCD_D7 | LCD_READ | rs | lcd->backlight;

	/* Set reading lines and reading mode */
	i2c_write_byte(lcd->_pi, lcd->_i2c_handle, lcd->_addr << 1 | I2C_WRITE);
	i2c_write_byte(lcd->_pi, lcd->_i2c_handle, lines);
	i2c_write_byte(lcd->_pi, lcd->_i2c_handle, lines | LCD_ENABLED);

	/* Actually read lines */
	i2c_write_byte(lcd->_pi, lcd->_i2c_handle, lcd->_addr << 1 | I2C_READ);
	byte = i2c_read_byte(lcd->_pi, lcd->_i2c_handle);
	// i2c_write_bit(lcd->_pi, lcd->_i2c_handle, I2C_NACK);

	/* Unset read mode */

	i2c_write_byte(lcd->_pi, lcd->_i2c_handle, lcd->_addr << 1 | I2C_WRITE);
	i2c_write_byte(lcd->_pi, lcd->_i2c_handle, lines & ~LCD_ENABLED);
	i2c_write_byte(lcd->_pi, lcd->_i2c_handle, lines & ~LCD_READ);
