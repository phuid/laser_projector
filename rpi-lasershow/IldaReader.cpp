...
bool IldaReader::checkHeader() {
    ...
    if (buf[7] != 0 && buf[7] != 1 && buf[7] != 2 && buf[7] != 4 && buf[7] != 5) {
        ...
        return false;
        throw "read format code " + ('0' + buf[7]) + "only supported formats 0,1,2,4,5 from section 3.2. \"Format Codes\" in spec: https://www.ilda.com/resources/StandardsDocs/ILDA_IDTF14_rev011.pdf";
    }
    else {
        this->current_format_code = buf[7];
    }
